...
describe('Asesor Ve vista 360 de un usuario', () => {
  it('Should login the user Asesor Succesfully', () => {
    ...
    cy.get('select#identification_type')
      .contains('Cédula de ciudadanía')
      .should('be.visible');
    cy.get('select#identification_type')
      .contains('Cédula de extranjería')
      .should('be.visible');
    cy.get('select#identification_type')
      .contains('Tarjeta de identidad')
      .should('be.visible');
    cy.get('select#identification_type')
      .contains('Pasaporte')
      .should('be.visible');
    ...
    cy.get('input#identification_number')
      .should('be.visible')
      .type('1234556');
...
describe('Asesor crea una PQR a un usuario', () => {
  it('should create a PQR Succesfully', () => {
    ...
    // Haz clic en la opción "Crear PQR"
    ...
    // Se verifican elementos gráficos propios de creación de PQR
    ...
    cy.get('textarea#description')
      .clear()
      .type('Descripción de prueba con más de 200 caracteres'.repeat(5));
...
describe('Asesor ve listado de PQRs asignado a sí mismo', () => {
  it('should show list of PQRs', () => {
    ...
    // Asesor ve listado de PQRs asignado a sí mismo
...
describe('Asesor crea PQR, y vuelve a la lista exitosamente. Nueva PQR no necesariamente debe aparecer en lista', () => {
  it('should create a PQR', () => {
    ...
    cy.contains('button', 'Crear PQR').should('be.enabled');
    // Se hace click en el botón crear PQR y se evidencia mensaje de éxito
    cy.contains('button', 'Crear PQR').click();
    cy.contains('La PQR ha sido creada exitosamente').should('be.visible');
...
describe('Asesor ve detalle de Usuario y vuelve a la lista', () => {
  it('should show user details', () => {
    ...
    cy.contains('Correo electrónico').should('be.visible');
    cy.contains('Número de identificación').should('be.visible');
    cy.contains('Listado de PQRs reportados por el consumidor').should('be.visible');
    cy.contains('Empresa').should('be.visible');
...
describe('Asesor busca una PQR específica con la barra de búsqueda', () => {
      ...
      cy.get('table tbody tr').then((rows) => {
        ...
        cy.wrap(rows[randomIndex])
          ...
          .then(() => {
            ...
            cy.wait(1000); // Espera a que se carguen los resultados de PQRs del usuario
    ...
    cy.get('table tbody tr').then((rows) => {
      ...
      selectedRow.within(() => {
        cy.get('td')
          ...
          .invoke('text')
          .then((subject) => {
            // Ajusta `eq(2)` según la posición de la columna
        ...
        cy.get('button.btn.btn-primary.btn-sm.rounded-pill')
          .should('be.visible')
          .click();
...
describe('Update Pcc', () => {
  ...
  it('should display the detail of a randomly selected PCC', () => {
    ...
    // Verifica el mensaje de error de longitud mínima
    ...
    cy.get('textarea[formcontrolname="reason"]').type(longText);
    // Verifica el mensaje de error de longitud máxima
    ...
    // Botón "Guardar Cambios" debe aparecer deshabilitado
    cy.contains('button', 'Guardar Cambios').should('be.disabled');

    // Ingresa texto válido
    const validText = 'A'.repeat(10);
    cy.get('textarea[formcontrolname="reason"]').type(validText);
    ...
    cy.get('select[formcontrolname="status"]')
      .contains('Pendiente por documentación')
      .should('be.visible');
    ...
    cy.get('select[formcontrolname="status"]')
      .contains('Cerrada sin solución')
      .should('be.visible');

    // Enfoca y desenfoca el campo para activar la validación
    cy.get('select[formcontrolname="status"]').focus().blur();

    // Verifica el mensaje de error de campo requerido
...
describe('Representante de Empresa ve reporte', () => {
  it('Should login the user Empresa Successfully', () => {
    // Empresa se loggea exitosamente
    cy.visit('/');
    cy.contains('button', 'Empresa').click();
    ...
    // Verificar los filtros de fechas
    cy.wait(500);
    ...
    cy.get('table.table.table-dark.table-hover.align-middle.text-center').within(() => {
      ...
      // Verificar que el botón de detalle está presente en las acciones
      cy.wait(500);
    ...
    // Selecciona una fila aleatoria y haz clic en el botón "Detalle"
    cy.wait(500);
    ...
    // Ingresar fechas en los campos correspondientes
    ...
    cy.get('table.table.table-dark.table-hover.align-middle.text-center').within(() => {
      ...
      // Verificar que no hay filas en el cuerpo de la tabla
